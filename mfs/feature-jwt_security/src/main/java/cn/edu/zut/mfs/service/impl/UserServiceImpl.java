...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<SimpleGrantedAuthority> getPermissions(String username) {
        Map<String, Object> params = new HashMap<>();
        params.put("username", username);
        User user=userDao.selectByMap(params).get(0);
        List<SimpleGrantedAuthority> permissions = new ArrayList<>();
        //permissions.addAll(roleRelationDao.getPermissionList(userId));
        userPermissionRelationDao.getPermissions(user.getId()).forEach(item -> permissions.add(new SimpleGrantedAuthority(item)));
        return permissions;
    }
